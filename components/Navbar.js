  ...
  return (
    <nav className='bg-gray-900 shadow-xl shadow-white text-white flex justify-between items-center px-4 md:h-16'>
      ...
      <Link className="logo font-bold text-lg flex justify-center items-center" href={"/"}>
        ...
        <span className='text-xl md:text-base my-3 md:my-0'>Buy Me A Expresso!</span>
      </Link>
      ...
      <div className='relative flex justify-center items-center  md:block gap-4'>
        ...
        {session && <button className='text-white w-fit bg-gradient-to-br from-slate-800 to-emerald-600 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 ' onClick={() => { signOut() }}>Logout</button>}
        {!session && <Link href={"/login"}>
          <button className='text-white bg-gradient-to-br from-slate-800 to-emerald-600 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 '>Login</button></Link>}
      </div>
    </nav>
  )
}
